	...
	public class ProximityRegistrationMediator extends SignalMediator
	{
		...
		private function handleRegistration():void
		{
			...
			if(password != confirmPassword) {
				...
				if(userName == null) {
					UI.showError("Enter your Username.");
				} else if(name==null) {
					UI.showError("Enter your Name.");
				} else if(email==null) {
					UI.showError("Enter your Email.");
				} else if(relatedUser==null && role == 3) {
					UI.showError("Enter your Teacher's Username. If you don't know it please ask your teacher.");
				}					
				register.dispatch(new RegistrationDetails("proximityRegistrationProvider",userName, name, password, email, role, relatedUser, getContextInfo(), Device.language));
			}
		}
